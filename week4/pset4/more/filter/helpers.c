...
// Auxiliary function to calculate g-axis weight for each color
void calcWeightedColor(RGBTRIPLE pixel, int height, int width, int pixel_x[3], int pixel_y[3]);
...
void edges(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0, j = 0; i < height; j++)
    {
        ...
        if (j - 1 >= 0)
        {
            calcWeightedColor(image[i][j - 1], center, center - 1, pixel_x, pixel_y);
        }
        ...
        if (i - 1 >= 0)
        {
            calcWeightedColor(image[i - 1][j], center - 1, center, pixel_x, pixel_y);
        }
        ...
        if (i - 1 >= 0 && j - 1 >= 0)
        {
            calcWeightedColor(image[i - 1][j - 1], center - 1, center - 1, pixel_x, pixel_y);
        }
        ...
        gRed = round(sqrt(pow(pixel_x[0], 2) + pow(pixel_y[0], 2)));
        gGreen = round(sqrt(pow(pixel_x[1], 2) + pow(pixel_y[1], 2)));
        gBlue = round(sqrt(pow(pixel_x[2], 2) + pow(pixel_y[2], 2)));
        ...
        new_image[i][j].rgbtRed = gRed;
        new_image[i][j].rgbtGreen = gGreen;
        new_image[i][j].rgbtBlue = gBlue;
...
void calcWeightedColor(RGBTRIPLE pixel, int height, int width, int pixel_x[3], int pixel_y[3])
{
    ...
    int weight_x = gx[height][width], weight_y = gy[height][width];
    int red = pixel.rgbtRed, green = pixel.rgbtGreen, blue = pixel.rgbtBlue;

    pixel_x[0] += red * weight_x;
    pixel_x[1] += green * weight_x;
    pixel_x[2] += blue * weight_x;

    pixel_y[0] += red * weight_y;
    pixel_y[1] += green * weight_y;
    pixel_y[2] += blue * weight_y;
